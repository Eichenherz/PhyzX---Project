...
Character::Character( const FVec2& pos )
	:
...
void Character::Draw( Graphics& gfx ) const
{
	...
	if( effectActive )
	{
		animations[(int)iCurSequence].DrawColor( (IVec2)pos,gfx,Colors::Red );
	}
	else
	{
		animations[(int)iCurSequence].Draw( (IVec2)pos,gfx );
	}
}

void Character::SetDirection( const FVec2& dir )
{
	if( dir.x > 0.0f )
	{
